...
function BuildOutList() {
  var res = []
  var tempList = DeepCopy(configObj.state.config.outbounds)
  var i = 0
  ...
  for (var sub in configObj.state.config.subscribe) {
    sub = DeepCopy(configObj.state.config.subscribe[sub])
    for (var out in sub['outbounds']) {
      out = sub['outbounds'][out]
      if (!out['enabled']) {
        continue
      }
      out['tag'] = sub['tag'] + "_" + out['tag'] + "_" + i.toString()
      res.push(out)
      i += 1
    }
  }
...
function ApplyCoreConfig() {
  ...
  SetOutboud(coreConfig, BuildOutList())
...
function TryParse(inputData) {
  try {
    ...
    console.error("failed to parse as uif" + error);
  }

  try {
    return V2rayN2UIF(inputData);
  } catch (error) {
    console.error("failed to parse as v2rayn" + error);
  }

  try {
    return clash2UIF(inputData);
  } catch (error) {
    console.error("failed to parse as clash" + error);
  }
