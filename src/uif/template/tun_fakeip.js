...
function UpdateUniqeTagAndID(boundConfig) {
  ...
  // update detour and outbound
  for (var item in usedTagMap) {
    item = usedTagMap[item]
    if (!item['enabled'] || !('dial' in item)) {
      continue
    }
    var id = item['dial']['detour']['id']
    item['dial']['detour']['tag'] = ''
    if (id.length == 0) {
      continue
    }
    id = id[id.length - 1]
    ...
    if (detour == null) {
      item['dial']['detour']['tag'] = detour['core_tag']
    }
  }
  return res
}
