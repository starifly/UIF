...
export var DEFAULT_DNS = "udp://8.8.8.8"
export var DEFAULT_IP_DNS = "udp://8.8.8.8"
...
function InitDetour(out) {
  if (!out['enabled'] || !('dial' in out)) {
    return
  }
...
function UpdateUniqeTagAndID(boundConfig) {
  var usedTagMap = {
    'freedom': 1,
    'dns-out': 1,
    'block': 1,
...
function SetAllOutboudFreedom(res) {
  var destList = []
...
function AddRouteList(res, routeList) {
  ...
  if (outList.length != 0) {
    ...
    var url = new URL(uif.state.apiAddress)
...
function BuildCoreConfig(uifConfig, boundConfig, useHttpApi) {
...
export function BuildShareCoreConfig(uifConfig, boundConfig) {
  ...
  if (uifConfig.dnsAddress == 'dhcp://auto') {
    uifConfig.dnsAddress = DEFAULT_DNS
  }
  ...
  // prepare inbound
  var tunIn = newDefaultTunIn('realip')
  tunIn['enabled'] = true
  boundConfig['inbounds'] = [tunIn]

  // for (var item in boundConfig['outbounds']) {
  //   item = boundConfig['outbounds'][item]
  //   if (['wireguard'].includes(item['protocol']) && item['enabled']) {

  //   }
  // }

  var coreConfig = BuildCoreConfig(uifConfig, boundConfig, false)
