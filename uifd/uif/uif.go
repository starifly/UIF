...
func ProxyHTTP2(dst string, port string) (string, error) {
...
func ProxyHTTPDirect(dst string) (string, error) {
...
func ProxyGet(dst string) string {
	...
	if dst == "" {
		...
		if err != nil {
			res.Status = 1
			res.Res, err2 = ProxyHTTPDirect(dst)
			WriteLog(err.Error())
			WriteLog(res.Res)
			if err2 == nil {
				res.Status = 0
...
func SetAutoStartup(enable bool) string {
	...
	var err error
	if IsWindows() {
		err = AutoStartup(enable) // service can not popup Web
	} else {
		if enable {
...
func UfwPort(port string) {
	if IsLinux() {
		cmd := exec.Command("ufw", "allow", port)
		err := cmd.Run()
		if err != nil {
			WriteLog("failed to config firewall: " + err.Error())
		} else {
			WriteLog("opened " + port)
		}
	}
}

func TryOpenLinuxPort(port string) {
	if IsLinux() {
		UfwPort(port + "/tcp")
		UfwPort(port + "/udp")
	}
