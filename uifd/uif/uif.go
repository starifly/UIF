...
func GetKey() string {
	...
	PrivateKey = ReadFileOneLine(path)
...
func GetInfo() string {
	...
	// m.Ip = GetOutboundIP()
	m.Ip = GetDefaultInterface().String()
...
func DownloadFile(filepath string, downloadURL string) error {
	temp, _ := strconv.Atoi(GetApiPort())
	if TCPPortCheck(temp) {
		proxyUrl, _ := url.Parse("http://127.0.0.1:" + GetApiPort())
		http.DefaultTransport = &http.Transport{Proxy: http.ProxyURL(proxyUrl)}
	}
